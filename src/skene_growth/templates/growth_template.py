...
def _build_prompt(
    ...
    return (
        ...
        "     * healthyBenchmark: A benchmark value indicating what's considered healthy (e.g., \"> 25%\", \"< 4 hours\")\n\n"
...
def _validate_template_structure(template: dict[str, Any]) -> None:
    ...
    logger.info(f"✓ All lifecycles include required metrics arrays")
...
async def generate_growth_template(
    ...
    logger.success(f"Generated template: {parsed.get('title', parsed.get('name', 'Unknown'))} with {len(parsed.get('lifecycles', parsed.get('keywordMappings', {})))} stages")
    return parsed
