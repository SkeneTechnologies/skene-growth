...
def show_config_status(cfg, project_cfg, user_cfg):
    ...
    from skene.config import resolve_upstream_api_key_with_source

    console.print(Panel.fit("[bold blue]Configuration[/bold blue]", title="skene"))
    ...
    table.add_row(
        "Project",
        str(project_cfg) if project_cfg else "./.skene.config",
        "[green]Found[/green]" if project_cfg else "[dim]Not found[/dim]",
    )
    table.add_row(
        "User",
        str(user_cfg) if user_cfg else "~/.config/skene/config",
