...
- **Upstream push can be a genuine blocker.** Ask the user in chat to confirm before running `uvx skene push` — it POSTs `skene/engine.yaml` and the latest trigger migration to Skene Cloud (it does not generate SQL; run `uvx skene build` first if migrations are missing).
...
3. **Ask user to confirm**
   Tell the user: "`uvx skene push` uploads your engine and latest `*_skene_triggers.sql` to upstream. Proceed?"
...
5. **Report result**
   Engine path, trigger migration path, next step: apply DB migrations locally if needed (`supabase db push`), then "Run `/skene-status` to verify."
